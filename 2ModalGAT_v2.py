...
class GAT_FP(nn.Module):
    def __init__(self, in_size, hid_size, out_size, reconstruct, wFP):
        super().__init__()
        gcv = [in_size, 256, 8]
        self.num_heads = 4
...
if __name__ == "__main__":
    ...
    parser.add_argument('--prePath', help='prepath to directory contain DGL files', default='./2modals/')
    parser.add_argument('--modals', help='modals', default='avl')
    ...
    for test in range(args.numTest):
        ...
        if os.path.isfile(graphPath):
            ...
            data = IEMOCAP(missing = args.missing, nameDataset = args.dataset, path = dataPath, modals = args.modals)
        ...
        model = GAT_FP(in_size, 128, out_size, args.reconstruct, args.wFP).to(device)    
        print(model)
        # model training
        print("Training...")
